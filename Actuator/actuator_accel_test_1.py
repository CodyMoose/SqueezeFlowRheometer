...
def foreground():
	...
	print("This is gonna take a bit. At least {:d} minutes".format(math.ceil(osc_length*2*len(osc_mags)*len(osc_omegas)*len(osc_strengths)/60)))
	...
	for A in osc_mags:
		for omega in osc_omegas:
			for strength in osc_strengths:
				print("Position oscillation for {:d} seconds: A = {:d}, omega = {:d}, strength = {:%}".format(osc_length, A, omega, strength))
				tic.set_max_decel(500000)
				tic.set_max_accel(500000)
				move_to_pos(center + A)

				# Now restrict maximum acceleration
				a_max = A*omega**2 * 100 # steps/s/100s, the units the controller uses for acceleration
	...
	for A in osc_mags:
		for omega in osc_omegas:
			for strength in osc_strengths:
				print("Velocity oscillation for {:d} seconds: A = {:d}, omega = {:d}, strength = {:%}".format(osc_length, A, omega, strength))
				...
				# Now restrict maximum acceleration
				a_max = A*omega/100 # steps/s/100s, the units the controller uses for acceleration
