...
def foreground():
	...
	print("Oscillating for {:d} seconds. Should be +/- 5mm in either direction".format(osc_length))
	...
	print("Velocity oscillating for {:d} seconds. Should be +10mm in one direction, and then back a few times".format(osc_length))
	while osc_time < osc_length: 
		osc_time = time() - osc_start
		# osc_pos = osc_mag * math.sin(osc_omega * osc_time) + center
		osc_pos = osc_mag * (1 - math.cos(osc_omega * osc_time)) + center
		tic.set_target_velocity(math.floor(osc_pos))
		tic.reset_command_timeout() # you have to give it a heartbeat at least every second
