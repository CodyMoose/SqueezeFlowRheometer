...
if __name__ == "__main__":
    ...
    with open("data/" + csv_name, "a") as datafile:
        ...
        datafile.write(
            "Current Time (s), Elapsed Time (s), Current Position (steps), Target Position (steps), Current Position (mm), Current Velocity (steps/10000s), Target Velocity (steps/10000s), Max Speed (steps/10000s), Max Decel (steps/100s^2), Max Accel (steps/100s^2), Step Mode, Voltage In (mV)\n"
        )
...
def foreground():
    ...
    for A in osc_mags_mm:
        for omega in osc_omegas:
            for strength in osc_strengths:
                print(
                    "Position oscillation for {:d} seconds: A = {:.1f}, omega = {:d}, strength = {:%}".format(
                ...
                a_max = (
                    A * omega**2 * 100 * microstep_ratio * 100
    ...
    for A in osc_mags_mm_s:
        for omega in osc_omegas:
            for strength in osc_strengths:
                print(
                    "Velocity oscillation for {:d} seconds: A = {:.1f}, omega = {:d}, strength = {:%}".format(
                        osc_length, A, omega, strength
                    )
                )
                tic.set_max_decel(500000 * microstep_ratio)
