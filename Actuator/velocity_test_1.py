...
def set_vel_to_pos(vel,pos):
	print("current planning mode: {:d}".format(tic.variables.planning_mode))
	if (pos - tic.variables.current_position) / vel > 0:
		tic.set_target_velocity(vel)
		print("current planning mode: {:d}".format(tic.variables.planning_mode))
		print("target velocity: {:d}".format(tic.variables.target_velocity))
		sleep(0.1)
		tic.reset_command_timeout()
		s = math.copysign(vel,1)
		while s*(tic.variables.current_position - pos) < 0:
			sleep(0.1)
			tic.reset_command_timeout()
			print("tv: {:d}, cv: {:d}, cp: {:d}, tp: {:d}".format(tic.variables.target_velocity,tic.variables.current_velocity,tic.variables.current_position,tic.variables.target_position))
		print("reached target {:d}, current pos is {:d}".format(pos,tic.variables.current_position))
	else:
...
def background():
	...
	while True:
		...
		if (time() - start_time) >= 60 or ((not f.is_alive()) and (time() - start_time) > 1):
			print("end of print")
			break
