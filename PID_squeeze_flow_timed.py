...
if __name__ == "__main__":
    ...
    dur_line = input(
        "Enter the test duration in seconds. Simply press Enter for the default of {:}s: ".format(
            default_duration
        )
    )
    if "config" in dur_line.lower() or len(dur_line) <= 0:
        ...
        temp = re.compile("[0-9.]+")
        res = temp.search(dur_line).group(0)
        test_duration = abs(float(res))
    print("Test duration is {:.2f}s".format(test_duration))
